...
CCamera::CCamera()
{
	...
	m_FixPosR = INIT_D3DXVECTOR3;
	m_bRot = false;						// 視点移動フラグ
}
...
void CCamera::Update(void)
{
	...
	if (pInputMouse->GetPress(CInputMouse::MOUSE_LEFT) && pInputMouse->GetPress(CInputMouse::MOUSE_RIGHT))
	{// マウスの左・右が押されている間

		m_Info.posV.x += pInputMouse->GetCursorMove().x;
		m_Info.posV.z -= pInputMouse->GetCursorMove().y;
		m_Info.posR.x += pInputMouse->GetCursorMove().x;
		m_Info.posR.z -= pInputMouse->GetCursorMove().y;
	}
	else if (pInputMouse->GetPress(CInputMouse::MOUSE_RIGHT))
	{// マウスの右ボタンが押されている間

		if (!m_bRot)
		{
			m_FixPosR = m_Info.posR;
			m_bRot = true;
		}
		// カーソルの移動量に応じて回転
		...
		{
			AxisRotationCamera(DIRECTION_UP, pInputMouse->GetCursorMove().y * CAMERA_ROT_FORCE_BY_CURSOR.y);
		}
		AxisRotationCamera(DIRECTION_LEFT, pInputMouse->GetCursorMove().x * CAMERA_ROT_FORCE_BY_CURSOR.x);
		...
		// 視点の位置を設定
		m_Info.posV.x = m_FixPosR.x + (sinf(m_Info.rot.y) * (m_Info.fDistance * (1.0f - fabsf(m_Info.fHeight))));
		m_Info.posV.y = m_FixPosR.y + (m_Info.fDistance * m_Info.fHeight);
		m_Info.posV.z = m_FixPosR.z + (cosf(m_Info.rot.y) * (m_Info.fDistance * (1.0f - fabsf(m_Info.fHeight))));
	}
	else
	{
		m_bRot = false;
	...
	m_Info.posV.y += (pInputMouse->GetWheel() / 5);
	m_Info.posR.y += (pInputMouse->GetWheel() / 5);
